...
import stocksFakerFactory from "./stocksFakerFactory";
...
const StocksFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = stocksFakerFactory(props.user,fakerCount,productIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("stocks").create(elem));
...
export default connect(mapState, mapDispatch)(StocksFakerDialogComponent);
