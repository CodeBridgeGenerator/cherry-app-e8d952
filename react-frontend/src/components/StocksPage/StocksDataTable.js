...
const StocksDataTable = ({ items, onEditRow, onRowDelete, onRowClick }) => {
    const dt = useRef(null);
    const urlParams = useParams();
    const [showUpload, setShowUpload] =useState(false)

const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.product?.name}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.qty}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.stockdate}</p>
...
<Column field="product" header="product" body={dropdownTemplate0} style={{ minWidth: "8rem" }} />
<Column field="qty" header="Qty" body={pTemplate1} sortable style={{ minWidth: "8rem" }} />
<Column field="stockdate" header="Stockdate" body={pTemplate2} sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Stocks Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>
        </>
    );
